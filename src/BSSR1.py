...
# replace the following two lines of code with the previous line if the tqdm package is not installed
    for i in tqdm_notebook(range(len(files))):
        ...
        read_data_copy = read_data[2:-2]

        read_data_copy = [int(float(numeric_string)) for numeric_string in read_data_copy]
        #print(read_data_copy, type(read_data_copy))
        sims = read_data[2:-2].astype(np.float)
        sims = sims.astype(np.int)
        #sims = read_data_copy
...
def df2sim_subsample(df, column_names, nr_individuals = 1000):
    ...
    similarity_matrix = df.loc[random_names.astype(int),random_names]
    #similarity_matrix = [int(fnum) for fnum in similarity_matrix]
